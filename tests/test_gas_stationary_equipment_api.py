...
def test_mixed_compressor_facade_exposes_governed_solver_and_evidence_contracts() -> None:
    ...
    assert mixed_api.ACTIVE_COMPRESSOR_P2_NUMERICAL_REPRESENTATION_REF.startswith(
        "numerics://gas/"
    )
    assert mixed_api.MIXED_STATIONARY_GAS_EXPORT_VERSION.startswith("phase6-gas/")
    assert mixed_api.STATIONARY_EQUIPMENT_BENCHMARK_EVIDENCE_SCHEMA_VERSION.startswith(
        "phase6/"
    )
