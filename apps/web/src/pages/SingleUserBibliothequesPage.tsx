...
export function SingleUserBibliothequesPage() {
  ...
  const itemsQuery = useQuery({
    ...
    queryFn: () =>
      apiRequest<Page<CatalogItem>>(
        "/catalog/" + collection + "?limit=500&offset=0&organization_id=" + organizationId,
      ),
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      resetStructured();
      await queryClient.invalidateQueries({ queryKey: ["catalog", organizationId, collection] });
    },
  });
  ...
  const error = itemsQuery.error ?? createMutation.error ?? versionMutation.error;
  const success = createMutation.isSuccess || versionMutation.isSuccess;

  return (
    <div className="stack">
      {error ? <ErrorNotice error={error} /> : null}
      {success ? <SuccessNotice>La ressource a été enregistrée et rendue disponible.</SuccessNotice> : null}

      <Panel
        title="Catalogue technique"
        description="Chaque ressource possède une source, une empreinte et un historique de versions. Aucun circuit d'approbation séparé n'est requis."
      >
      ...
      <Panel
        ...
        description={items.length + " version(s) disponible(s)."}
      >
        {items.length ? (
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td><StatusBadge value={item.status === "archived" ? "archived" : "available"} /></td>
          ...
          <EmptyState title="Catalogue vide" detail="Créez la première ressource à partir d'une donnée vérifiable." />
        )}
      </Panel>

      <Panel
        title="Nouvelle ressource"
        description="Les propriétés sont validées par le backend scientifique puis la ressource devient immédiatement utilisable par l'ingénieur."
      >
        <form
        ...
        >
          ...
          <div className="button-row">
            <button type="button" className="button button-ghost" onClick={() => setExpertMode((current) => !current)}>
          ...
          {structuredProblems.length ? (
            <div className="notice notice-error" role="alert">
              <ul className="issue-list negative">
                {structuredProblems.map((problem) => <li key={problem}><span>{problem}</span></li>)}
              </ul>
            </div>
          ) : null}
          ...
          <div className="button-row">
            <button
              ...
              disabled={!organizationId || createMutation.isPending || structuredProblems.length > 0}
            >
              Enregistrer
            </button>
            {useStructuredForm ? (
              <button className="button button-ghost" type="button" onClick={resetStructured}>Réinitialiser la fiche</button>
            ) : (
              <button className="button button-ghost" type="button" onClick={() => setPayload(examplePayload(collection))}>
                Restaurer l'exemple
              </button>
            )}
          </div>
