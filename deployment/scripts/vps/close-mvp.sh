#!/usr/bin/env bash
# Ferme les portes techniques du MVP sans prétendre remplacer la recette métier.
#
...
#   PREPARE_REFERENCE       1 pour créer REF-MVP-01, 0 s'il existe déjà
...
if [[ "${PREPARE_REFERENCE}" == "1" ]]; then
  printf '== Construction du dossier REF-MVP-01 (ingénieur unique) ==\n'
...
Reste volontairement MANUEL avant v1.0.0-mvp :
  1. examen et signature de docs/validation/acceptation_ingenieur_mvp.md par l'ingénieur utilisateur ;
...
Aucun compte Approver n'est requis dans le MVP mono-utilisateur.
