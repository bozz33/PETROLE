...
def test_approved_state_requires_approval_and_draft_rejects_active_approval() -> None:
    with pytest.raises(ValueError, match="APPROVED.*approbation"):
        _criterion(approval_ref=None)

    with pytest.raises(ValueError, match="DRAFT.*approbation"):
...
def test_materialization_rejects_duplicate_criteria_or_observation_assignment() -> None:
    ...
    with pytest.raises(ValueError, match="identifiants de critères.*uniques"):
        materialize_approved_gas_benchmark_criteria(
            context=_CONTEXT,
